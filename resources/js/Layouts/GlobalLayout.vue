  ...
  <div class="relative flex flex-col bg-page min-h-screen" style="scrollbar-gutter: stable;">
    ...
    <header class="bg-neutral text-text-light shadow" v-if="$slots.header">
      ...
      <div v-if="heroImage" class="flex justify-center items-center  h-96 space-x-3 mx-auto py-6 px-4 sm:px-6 lg:px-8 "
        :style="{ backgroundImage: `linear-gradient(to top, rgba(0, 0, 0, 0) 40%, rgba(0, 0, 0, 0.5) 80%, rgba(0, 0, 0, 0.75) 100%), url(${heroImage})`, backgroundPosition: '50% 10%', backgroundSize: 'cover', backgroundRepeat: 'no-repeat' }">
    ...
    <footer class="mt-20">
      ...
      <div class="grid sm:grid-cols-2 bg-dark min-h-64 py-14 font-bold text-xl text-text-light">
        <div class="flex flex-col justify-around items-center sm:items-start ms-0 sm:ms-6 h-full">
          <div class="flex justify-between text-2xl w-1/2 mx-auto sm:mx-0">
