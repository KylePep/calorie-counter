...
<template>
  ...
  <div v-if="displayMode == 'list'" :class="index % 2 == 0 ? 'border-accent/50' : 'border-light'"
    class="flex flex-col min-h-40 min-w-40 bg-white p-4 text-center border-2 rounded  bg-main shadow-lg my-4">

    <div class="flex flex-1 flex-col justify-between space-y-3">

      <div class=" grid grid-cols-2 sm:grid-cols-4 gap-y-3">
        <p class="order-1 font-bold text-lg">{{ getDayOfWeek(new Date(calorieDay.created_at).getDay()) }}, {{
          formattedDate() }}</p>
        <div class="order-3 sm:order-2 col-span-2 flex flex-col">
          <h2 class="text-lg">Calories: {{ calorieDay.count }}</h2>
          <h3 class="text-lg">Goal: {{ calorieDay.goal }}</h3>
          <h4 class="text-lg">BMR: {{ calorieDay.bmr }}</h4>
        </div>
      ...
      <div>
        <h3 v-if="calorieDay.food_items.length != 0" class="font-bold">Food Eaten</h3>
