...
const topBannerPage = computed(() => {
  const allowedTopPages = ['/', '/history', '/account'];
  return allowedTopPages.some(title => page.url == title);
});
</script>

<template>
  <div v-if="position == 'footer' || topBannerPage"
    ...
    <Link :href="route('calculator')"
      class="text-base text-accent hover:text-accent-yellow font-bold uppercase bg-white px-28 sm:px-32 py-4 duration-300">get
    your bmr!
    </Link>
  </div>
